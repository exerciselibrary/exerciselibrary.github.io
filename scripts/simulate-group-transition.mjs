// simulate-group-transition.mjs - test the group mode re-enable logic
...
class SupersetExecutorV2Sim {
  ...
  isGrouped(itemIndex) {
    for (const group of this.groups) {
      if (group.isGroup && group.items.some(e=>e.index===itemIndex)) return true;
    }
    return false;
  }
...
let ti=0;
let groupExecutionMode = true; // Start with group mode enabled
const events=[];
...
while (ti < timeline.length) {
  ...
  events.push(`[ti=${ti}] RUN ${item.name||item.type} set ${entry.set} (groupMode=${groupExecutionMode})`);

  if (groupExecutionMode && exec.isGrouped(entry.itemIndex)) {
    const next = exec.getNextExercise(entry.itemIndex);
    events.push(`  → executor returns ${JSON.stringify(next)}`);
    ...
    if (next.action==='next-exercise'){
      ...
      if (idx!==-1) {
        events.push(`  → jump to ti=${idx} (${planItems[next.itemIndex].name} set ${setToRun})`);
        ti=idx;
        continue;
      }
    }
    if (next.action==='rest-then-continue'){
      ...
      const idx = findTimelineIndexFor(next.itemIndex,setToRun);
      if (idx!==-1) {
        events.push(`  → rest then jump to ti=${idx} (${planItems[next.itemIndex].name} set ${setToRun})`);
        ti=idx;
        continue;
      }
    }
    if (next.action==='complete') {
      ...
      const lastIdx = findGroupLastTimelineIdx(group);
      events.push(`  → group's last timeline idx=${lastIdx}, moving ti from ${ti} to ${lastIdx}`);
      ti = lastIdx;
      // Do NOT disable groupExecutionMode here; let the upcoming item check decide
    }
  }
  ...
  // Check if upcoming item is grouped, re-enable/disable group mode accordingly
  if (ti < timeline.length) {
    const upcomingEntry = timeline[ti];
    const upcomingItemIndex = upcomingEntry.itemIndex;
    const wasGroupMode = groupExecutionMode;
    groupExecutionMode = exec.isGrouped(upcomingItemIndex);
    if (wasGroupMode !== groupExecutionMode) {
      events.push(`  → upcoming [ti=${ti}] item ${planItems[upcomingItemIndex].name} is grouped=${groupExecutionMode}, toggling mode`);
    }
  }
}
...
console.log('\n=== Events Log ===\n' + events.join('\n'));
console.log('\n=== Summary ===');
console.log('Group 2 should execute as: superset2#1 → superset2b#1 → superset2#2 → superset2b#2 → superset2#3');
